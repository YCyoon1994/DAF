...
public class Main{
    public static void main(String[] args) {
        ...
        Scanner scanner = new Scanner(new File(dataGraphFile));
        String str = scanner.nextLine();
        String [] str_ = str.split(" ");
        int num_of_vertice_data =  Integer.parseInt(str_[2]);

        int[] data_label_freq = new int [100];//save number of label frequency;
        int[] data_degree = new int[num_of_vertice_data];//save the degree of data vertex
        int[] data_label = new int[num_of_vertice_data]; //save ith intergs label

        while(scanner.hasNextLine()){
          str = scanner.nextLine();
          str_ = str.split(" ");
          if(str_[0].equals("v")){
            int n =  Integer.parseInt(str_[1]);
            int m =  Integer.parseInt(str_[2]);
            data_label_freq[m]++;
            data_label[n] = m;
          }
          else{
